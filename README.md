...
| `DEFAULT_LANGUAGE` | резервный язык ответа для неоднозначного текста и язык TTS по умолчанию (`ru`/`kk`/`en`); значение по умолчанию остаётся `ru`, STT без `language` использует `auto` |
