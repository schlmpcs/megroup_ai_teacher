...
- **Голос**: STT и TTS обслуживает встроенный GPU-сайдкар `voice` (Whisper
  ...
  MMS как fallback, каталоги `./voice` и `./voice_omnivoice`); полный конвейер
...
FastAPI-прокси (этот репозиторий)            ┌─ scenarios/*.json  (контекст сцен)
   ...
   │          + Supertonic default / Qwen3-TTS 0.6B selectable (ru)
   │          + OmniVoice fixed young-male default / MMS fallback (kaz)
   ▼
