...
FastAPI-прокси (этот репозиторий)            ┌─ scenarios/*.json  (контекст сцен)
   ...
   │          + Supertonic default / Qwen3-TTS 0.6B selectable (ru) / MMS (kaz)
   ▼
...
| POST | `/tts` | `{text, language?, backend?, voice?, format?, instructions?}` → WAV; русский `backend` = `supertonic` (default) / `qwen` / `mms` |
| POST | `/voice_ask` | multipart `file` + необязательный `language=ru/kk/auto`, `tts_backend` и поля сценария, состояния и лаборатории → `{question, language, answer, citations, audio_base64}` или SSE; без `language` STT автоопределяет RU/KK и этот язык используется для TTS |
