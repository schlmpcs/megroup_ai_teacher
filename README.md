...
- **Эмбеддинги:** локальный GPU-сайдкар `embedder` на модели **BAAI/bge-m3**
  (мультиязычная, поддерживает русский, казахский и английский; FlagEmbedding `BGEM3FlagModel`,
...
- **Голос**: STT и TTS обслуживает встроенный GPU-сайдкар `voice` (Whisper
  ru/kk/en/auto + Supertonic по умолчанию / Qwen3-TTS 0.6B по явному выбору на русском и английском,
...
FastAPI-прокси (этот репозиторий)            ┌─ scenarios/*.json  (контекст сцен)
   ...
   │       ├─ Whisper (ru/kk/en/auto) POST http://voice:8001/tts/synthesize?format=wav
   │       ├─ Supertonic / Qwen / MMS (ru)
   │       ├─ Supertonic / Qwen, shared instances (en)
   │       └─ MMS fallback (kk)
...
`ADMIN_UI_COOKIE_SECURE=true`. Compose передаёт сервису `admin-ui` оба серверных
API-ключа; браузер получает только защищённую сессию и
CSRF-токен. Одна панель содержит загрузку, задания, документы и вкладку `Тест
API` для `/health`, `/ready`, `/ask`, streaming chat, `/hint`, `/stt`, `/tts` и
`/voice_ask`. Удаление отсутствующих документов доступно только при полном
сканировании с пустым относительным путём. Загруженные файлы и история задания
хранятся до удаления завершённого задания из панели.
...
Английская структура поддерживается тем же загрузчиком:

```text
Corpus/
├── School materials/Biology/en/Biology Grade 9.epub
└── Laboratory works/Physics/Physics Grade 10/en/Lab work No. 2.docx
```

Правила распознавания путей:

- каталог типа документа должен начинаться с `Школьный материал` или называться
  `Лабораторные работы`; английские aliases: `School materials`, `Textbooks`,
  `Laboratory works`, `Lab instructions`;
- предмет должен быть `Физика`, `Химия`, `Биология`, `Physics`, `Chemistry` или
  `Biology`;
- для лабораторной инструкции класс берётся из каталога вида
  `Физика 10 класс`, а для учебника из имени файла;
- язык определяется по каталогу `рус`, `русс`, `каз`, `қаз`, `en`, `eng` или
  `english`;
- номер лабораторной работы берётся из первого номера в имени файла, например
  `Лабораторная работа №2.docx`, `Зертханалық жұмыс № 2.docx` или
  `Lab work No. 2.docx`.

Из этих значений формируется стабильный `lab_id`. Например, путь выше получает
`physics-10-ru-02`; английский пример получает `physics-10-en-02`. Тренажёр
должен передать те же значения в объекте `lab`.
...
нужны Tesseract и языковые модели `rus`/`kaz`/`eng`. Они включены в Docker-образ API,
...
Tesseract даже при формально большом количестве распознанных слов.
...
| `lang` | `ru`, `kk` или `en` | язык структурированного документа |
...
| `VOICE_TTS_EN_DEFAULT_BACKEND` | backend для английского TTS без поля `backend`: `supertonic` по умолчанию или `qwen` |
...
| `NVIDIA_PERSISTENCE_MODE` | `1` просит драйвер включить persistence mode при каждом старте voice-контейнера; ошибка не блокирует запуск приложения |
| `STT_KEEP_WARM_ENABLED` | запускать реальные RU/KK STT-пробы при старте и после периода без пользовательских запросов; по умолчанию `true` |
| `STT_KEEP_WARM_REAL_IDLE_S` | сколько секунд без завершённого пользовательского STT должно пройти до фоновой пробы; по умолчанию `180` |
| `STT_KEEP_WARM_INTERVAL_S` | минимальный интервал между успешными фоновыми прогревами; по умолчанию `240` |
| `STT_KEEP_WARM_POLL_S` | частота проверки необходимости прогрева; по умолчанию `15` |
...
| `TTS_EN_BACKEND` | английский default внутри voice-сайдкара; использует уже загруженный `supertonic` или `qwen` |
...
| `TTS_SUPERTONIC_MODEL_DIR` | постоянный каталог общих RU/EN Supertonic assets; compose использует `/models/hf_cache/supertonic3` в volume `voice_hf_cache` |
...
| `TTS_NORMALIZE_EN_NUMBERS` | безопасно раскрывать английские числа, единицы, даты, время, проценты и температуры; по умолчанию `true` |
| `DEFAULT_LANGUAGE` | резервный язык ответа после явного `language`, текущего текста, недавней истории и `lab.lang`; также язык TTS по умолчанию (`ru`/`kk`/`en`), STT без `language` использует `auto` |

### Нормализация чисел для TTS

Русский, казахский и английский маршруты используют отдельные детерминированные нормализаторы
...
Voice image pins `supertonic==1.3.1`, the tested release with official `ru` and
`en` support. English and Russian reuse one loaded Supertonic instance and one
loaded Qwen instance, so English does not duplicate those large models in memory.
Supertonic files live in the persistent `voice_hf_cache` volume rather than the
...
Heat two hundred fifty milliliters to twenty five degrees Celsius.
The solution is twelve point five percent.
H2O, LAB-204, report2.csv, and URLs remain unchanged.
```
...
|------|------|-----------|
| POST | `/ask` | Вопрос с грунтингом: `{query, language?, conversation_id?, scenario_id?, scenario_state?, lab?}` → ответ + `language` + `citations` + `conversation_id` |
| POST | `/v1/chat/completions` | OpenAI-совместимый чат (`stream` поддерживается), расширен полями `language=ru/kk/en`, `scenario_id`, `scenario_state`, `lab` |
| POST | `/hint` | Перефразирование подсказки: `{hint_text, hint_level, language?, scenario_id?, scenario_state?}` с возвращаемым `language` |
| POST | `/stt` | multipart `file`, необязательный `language=ru/kk/en/auto` → `{text, language}`; без `language` включается автоопределение |
| POST | `/tts` | `{text, language?, backend?, voice?, format?, instructions?}` → WAV; русский = `supertonic` / `qwen` / `mms`, английский = `supertonic` (default) / `qwen`, казахский = `omnivoice` (default) / `mms` |
| POST | `/voice_ask` | multipart `file` + `language=ru/kk/en/auto`, optional `response_language=ru/kk/en`, `tts_backend` и поля лаборатории → JSON или SSE с language metadata |
...
English uses canonical code `en`. `language` selects the answer language, while
`lab.lang` continues to select the preferred corpus language and exact `lab_id`.
They may differ, for example `language=en` with `lab.lang=ru` produces an English
answer grounded in the Russian procedure. The default remains Russian.
...
curl -s localhost:8000/ask \
  ...
  -H 'Content-Type: application/json' \
  -d '{
    "query": "What is the purpose of this lab?",
    "language": "en",
    "lab": {"subject": "physics", "grade": 10, "lang": "en", "lab_number": 2}
  }'
```

The same `language=en` extension is supported by `/v1/chat/completions` and
`/hint`. STT accepts `en` or `auto`; TTS accepts English with Supertonic by
default or Qwen by explicit selection. `/voice_ask` accepts STT `language` plus
an optional `response_language`. Structured uploads accept `lang=en`.

See [the English migration and operations runbook](docs/english-support-runbook.md)
for complete curl examples, corpus layout, ingestion, health checks, and rollout
steps. Code support does not supply authoritative English textbooks or procedures.
Those source files must be provided and ingested for complete production grounding.
...
Поля см. в `scenarios/physics_lab_02_heating.json`: `scenario_name`, `language`, `subject`,
...
Английский демонстрационный вариант находится в
`scenarios/physics_lab_02_heating_en.json`; `/admin/scenarios` возвращает язык,
если он объявлен в JSON.
...
Для ручной проверки войдите в панель оператора и откройте вкладку `Тест API`.
Она проверяет health/readiness, текстовые запросы, streaming, STT, TTS и полный
`voice_ask`-конвейер через серверный прокси, не раскрывая API-ключ браузеру.
Вызовы `/ask`, chat, `/hint` и `/voice_ask` используют OpenAI и расходуют токены.
