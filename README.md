...
- **Голос**: STT и TTS обслуживает локальный GPU-сайдкар (Whisper ru/kk/auto +
  MMS/Silero ru/kk, см. `../vrrag_ttsstt`); полный конвейер `/voice_ask` под
  целевую задержку ≤5 c (ТЗ §5, §7).
...
- **≤5 c**: `/voice_ask` возвращает по-стадийные задержки (`stt`/`llm`/`tts`).
  STT/TTS обслуживает локальный сайдкар (`VOICE_BASE_URL`, см. `../vrrag_ttsstt`);
