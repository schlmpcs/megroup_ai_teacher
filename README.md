...
- **Голос**: STT и TTS обслуживает встроенный GPU-сайдкар `voice` (Whisper
  ru/kk/auto + Qwen3-TTS 0.6B по умолчанию / Supertonic для сравнения на русском,
  MMS на казахском, каталог `./voice`); полный конвейер
...
FastAPI-прокси (этот репозиторий)            ┌─ scenarios/*.json  (контекст сцен)
   ...
   │          + Qwen3-TTS 0.6B default / Supertonic (ru) / MMS (kaz)
   ▼
...
| `ALLOW_GENERAL_KNOWLEDGE_FALLBACK` | разрешить ответ из общеизвестных научных знаний без цитат, только если найденные материалы непригодны или не отвечают на вопрос и нет авторитетного контекста сцены/процедуры |
...
| `VOICE_TTS_RU_DEFAULT_BACKEND` | backend, который API явно отправляет для русского TTS без поля `backend`; по умолчанию `qwen` |
| `TTS_RU_BACKEND` / `TTS_RU_BACKENDS` | default и список загружаемых русских backend-ов voice-сайдкара; по умолчанию `qwen` и `qwen,supertonic` |
| `TTS_RU_QWEN_MODEL` | Hugging Face ID Qwen3-TTS; по умолчанию `Qwen/Qwen3-TTS-12Hz-0.6B-CustomVoice` |
| `TTS_RU_QWEN_SPEAKER` | голос Qwen по умолчанию; `Serena` |
...
| POST | `/tts` | `{text, language?, backend?, voice?, format?, instructions?}` → WAV; русский `backend` = `qwen` (default) / `supertonic` / `mms` |
| POST | `/voice_ask` | multipart `file` + необязательный `tts_backend` и поля сценария, состояния и лаборатории → `{question, answer, citations, audio_base64}` или SSE |
