...
_UNIT_TOKEN = (
    r"mL|ml|kg|cm|mm|km|Hz|kHz|MHz|Pa|kPa|mol|sec|mins?|hours?|"
    r"min|ms|s|L|l|g|m|h|V|A|W|J|N"
)
...
_RANGE_UNIT_RE = re.compile(
    rf"(?<!\w)(?P<start>{_SIGNED_NUMBER})\s*(?:-|\u2013|\u2014)\s*"
    rf"(?P<end>{_SIGNED_NUMBER})\s*(?P<unit>{_UNIT_TOKEN})(?![A-Za-z])"
)
_RANGE_RE = re.compile(
    rf"(?<![\w.,])(?P<start>{_SIGNED_NUMBER})\s*(?:-|\u2013|\u2014)\s*"
    rf"(?P<end>{_SIGNED_NUMBER})(?!\w|[.,]\d)"
)
_UNIT_RE = re.compile(
    rf"(?<!\w)(?P<number>{_SIGNED_NUMBER})\s*"
    rf"(?P<unit>{_UNIT_TOKEN})(?![A-Za-z])"
)
_GROUPED_DECIMAL_RE = re.compile(
    r"(?<![\w.,])(?P<number>[−-]?\d{1,3}(?:,\d{3})+\.\d+)(?!\w|[.,]\d)"
)
_GROUPED_INTEGER_RE = re.compile(
    r"(?<![\w.,])(?P<number>[−-]?\d{1,3}(?:,\d{3})+)(?!\w|[.,]\d)"
)
...
def _pluralized(number_text: str, noun: str) -> str:
    singular = abs(float(_normalize_number(number_text))) == 1.0
...
def _replace_temperature(match: Match[str]) -> str:
    ...
    degree = "degree" if abs(float(_normalize_number(number))) == 1.0 else "degrees"
...
def _replace_range_unit(match: Match[str]) -> str:
    unit = _UNIT_NAMES[match.group("unit").casefold()]
    return (
        f"{_number_words(match.group('start'))} to "
        f"{_pluralized(match.group('end'), unit)}"
    )
...
def _replace_range(match: Match[str]) -> str:
    return (
        f"{_number_words(match.group('start'))} to "
        f"{_number_words(match.group('end'))}"
    )
...
def normalize_english_text(text: str) -> str:
    ...
    normalized = _RANGE_UNIT_RE.sub(_replace_range_unit, normalized)
    ...
    normalized = _GROUPED_DECIMAL_RE.sub(
        lambda match: _number_words(match.group("number")), normalized
    )
    normalized = _GROUPED_INTEGER_RE.sub(
        lambda match: _number_words(match.group("number")), normalized
    )
