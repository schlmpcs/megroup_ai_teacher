...
def test_build_system_prompt_allows_current_subject_or_lab():
    ...
    assert "CURRENT SUBJECT AND LABORATORY SCOPE" in prompt
    assert "may cover any topic" in prompt
    assert "does not need to be directly related" in prompt
    assert "another school subject" in prompt
...
def test_ambiguous_followup_inherits_recent_conversation_language(monkeypatch):
    monkeypatch.setattr(llm.settings, "DEFAULT_LANGUAGE", "ru")
    history = [
    ...
    assert llm.resolve_answer_language("Continue", history=history) == "en"


def test_current_query_language_overrides_conversation_history(monkeypatch):
    monkeypatch.setattr(llm.settings, "DEFAULT_LANGUAGE", "ru")
    history = [
        {"role": "user", "content": "What is gravity?"},
        {"role": "assistant", "content": "Gravity attracts masses."},
    ...
    assert llm.resolve_answer_language("Объясни подробнее", history=history) == "ru"
...
def test_english_scope_allows_current_subject_topic():
    refusal = llm._lab_scope_refusal(
...
def test_english_scope_refusal_is_exact_for_different_subject():
    refusal = llm._lab_scope_refusal(
...
async def test_generate_answer_allows_same_subject_question_outside_exact_lab(
    ...
    assert "may cover any topic" in captured["instructions"]
